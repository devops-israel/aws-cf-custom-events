...
function createResource(event, context) {
  ...
  if (event.PhysicalResourceId && event.PhysicalResourceId !== undefined) {
    physicalResourceId = event.PhysicalResourceId;
  } else {
    physicalResourceId = `${event.StackId.replace(/^.*stack\/([^/]+)\/.*/g, '$1')}-${event.LogicalResourceId}-${exports.uniqueSuffix()}`;
  }
  ...
  const params = {
    ...
    Targets: {
      ...
      InputTransformer: optProp(event, 'InputTransformer'),
...
function deleteResource(event, context) {
  ...
  cwe.removeTargets({ Rule: optProp(event, 'Rule'), Ids: [event.PhysicalResourceId] }, deleteCallback);
}
